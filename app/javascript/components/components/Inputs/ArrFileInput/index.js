      ...
      {values && [...values].map((file, i) => {
        ...
        return (
          <div key={file.id || file.lastModified +  Math.random(10000)} className={styles.fileList}>
            ...
            {radio.isShown && radioParams && <Radio name={makeName(root, radio.actionRoot)} idx={idx} withoutChecked={radio.withoutChecked} root={root} index={i} actionRoot={radio.actionRoot} defaultValue={radio.defaultValue || (radio.defaultValue === 0 ? 0 : -1)} label={radioParams.label || ''} labelClass={styles.radio}/>}
          </div>
        )
      })}
    </div>
